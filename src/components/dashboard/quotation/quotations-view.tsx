...
import { formatDateTime, formatDateForInput, daysAgo } from "@/utils/format";
import type { Quotation, QuotationsResponse } from "@/types";


// ── Helpers ────────────────────────────────────────────────────────────────────
// fmt() delegates to formatDateTime from utils/format.ts
...
export default function QuotationsView() {
  ...
  const [startDate, setStartDate] = useState(formatDateForInput(daysAgo(7)));
  ...
  const fetchQuotations = useCallback(async () => {
    ...
    try {
      ...
      const data = await api.get<QuotationsResponse>(`/api/quotations?${params}`);
  ...
  const handleBulkDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.post(`/api/quotations/bulk-delete`, { ids: Array.from(selected) });
      setSelected(new Set());
      fetchQuotations();
    } catch (err) {
      alert(err instanceof ApiError ? err.message : "Error deleting quotations.");
    } finally {
      setDeleting(false);
    }
  };
  ...
  const handleSingleDownload = async (quotationNo: string) => {
    try {
      let logoBase64: string | null = null;
      try {
        const logoRes = await fetch("/logo.png");
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        handleSingleDownload(product.quotationNo);
                      }}
                      className="h-8 w-8 rounded-full border border-[#C5A059]/30 bg-card flex items-center justify-center text-[#C5A059] hover:bg-[#C5A059]/10 transition-colors"
                    >
                  ...
                  <TableCell className="text-center">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleSingleDownload(q.quotationNo);
                        }}
                        className="h-7 w-7 text-[#C5A059] hover:bg-[#C5A059]/10 rounded-full transition-all"
                        title={`Download ${q.quotationNo}`}
                      >
